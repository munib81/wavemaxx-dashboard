...
const CentralDeviceStatus=document.querySelector(".Status");
...
// setBattery(BatteryHandleAndSignal[CentralBattery],batterygood); 




// DoorOpenHandle(DoorHandle[RemoteDoor],StatusOpen,DoorSwitch1,"05:30 13/03/2023","05:30 13/03/2023");  
 //DoorOpenHandle(DoorHandle[CentraleDoor],StatusOpen,DoorSwitch2,"05:30 13/03/2023","05:30 13/03/2023" ); 
...
// SetPressure(PressureOutlet,4,"05:30 13/03/2023");
 ...
 //DeviceStatusShow(CentralDeviceStatus,DeviceActive);
...
function SetPressure(inlet,value,tiM)
{
    ...
    elem=  ValveHandle[inlet].querySelector("circle").style.strokeDashoffset=precentV ;
    ValveHandle[inlet].querySelector("p").innerHTML= Math.floor(BarPrecent*100)+"%";
    ValveHandle[inlet].querySelector("h2").innerHTML= value+"(<span>bar</span>)";
    console.log(elem);
...
function ValveButtonHandle(number)
{
    ...
    if(DeviceSelectSection[0].value=='0' ||DeviceSelectSection[1].value=='0')
        return;
  pubData="*#,22,1,2,"+(number)+",";  //,start,deviceId,cmdType,valvlenumber,statu
  topic="CentralR_"+DeviceSelectSection[0].value
  ...
  switch(number)
  {
    case 0:
        if(openF ==1)
             valveOpenHandle(MainValves,valveOn);
        else
            valveOpenHandle(MainValves,valaveOff);
        break;
    case 1:
        if(openF ==1)
            valveOpenHandle(MainValve,valveOn);
        else
            valveOpenHandle(MainValve,valaveOff);
        break;
    case 2:
        if(openF ==1)
            valveOpenHandle(Valve1,valveOn);
        else
            valveOpenHandle(Valve1,valaveOff);
        break;
      
    case 3:
        if(openF ==1)
            valveOpenHandle(Valve2,valveOn);
        else
            valveOpenHandle(Valve2,valaveOff);
        break;
    case 4:
        if(openF ==1)
            valveOpenHandle(Valve3,valveOn);
        else
            valveOpenHandle(Valve3,valaveOff);
        break;
    case 5:
        if(openF ==1)
            valveOpenHandle(Valve4,valveOn);
        else
            valveOpenHandle(Valve4,valaveOff);
        break;
    case 6:
        if(openF ==1)
            valveOpenHandle(Valve5,valveOn);
        else
            valveOpenHandle(Valve5,valaveOff);
        break;
    case 7:
        if(openF ==1)
            valveOpenHandle(Valve6,valveOn);
        else
            valveOpenHandle(Valve6,valaveOff);
        break;
    case 8:
        if(openF ==1)
            valveOpenHandle(Valve7,valveOn);
        else
            valveOpenHandle(Valve7,valaveOff);
        break;
        case 9:
            if(openF ==1)
                valveOpenHandle(Valve8,valveOn);
            else
                valveOpenHandle(Valve8,valaveOff);
            break;
...
function valveOpenHandle( valve_number,statusS)
{
    ...
    if(valve_number!=0)
        valve_ActualNum=valve_number+valveBaseVlue;
    console.log("setting valve");
    console.log(valve_number);
    console.log(statusS);
    if(statusS==0)
    {
        ValveHandle[valve_ActualNum].querySelectorAll("h1")[0].classList.add('active');
        ValveHandle[valve_ActualNum].querySelectorAll("h1")[1].classList.remove('active');

        if(valve_ActualNum==valveBaseVlue+1 || valve_ActualNum==0)
            ValveHandle[valve_ActualNum].querySelector("img").src="./images/gipc.png";
        else
            ValveHandle[valve_ActualNum].querySelector("img").src="./images/sprinklerC.png";
    }
    else
    {
        ValveHandle[valve_ActualNum].querySelectorAll("h1")[0].classList.remove('active');
        ValveHandle[valve_ActualNum].querySelectorAll("h1")[1].classList.add('active');
        if(valve_ActualNum==valveBaseVlue+1 || valve_ActualNum==0)
            ValveHandle[valve_ActualNum].querySelector("img").src="./images/gip.gif";
        else
            ValveHandle[valve_ActualNum].querySelector("img").src="./images/sprinkler.gif";
    }
...
//==============================================GSM Network Info==========================================
function SetCentralNetwork(hancle, value)
{
...
function SelectCentralDevice(inputType)
{
    ...
    if(inputType==1)
    {
        ...
        if(parseInt(DeviceSelectSection[0].value)==0 || parseInt(DeviceSelectSection[1].value)==0)
        {
            console.log("both 0");
            DisableControlMiddle[0].style.opacity =.30;
            DisableControlRight[0].style.opacity =.30;
        }
       else
       {
        console.log("dfdfdf");
    ...
    else if(inputType==2)
    {
       ...
       {     console.log("dfdfdf");
        ...
        var cENTRALtOPIC ="Central_"+DeviceSelectSection[0].value;
        topics[0]=cENTRALtOPIC;
...
var destinationName="Central_"+DeviceSelectSection[0].value;
...
SetValve(0,MainValveS0);
    ...
    DoorOpenHandle(DoorHandle[CentraleDoor-1],CentLimit1|CentLimit2,DoorSwitch1,"05:30 13/03/2023","05:30 13/03/2023");
     DoorOpenHandle(DoorHandle[CentraleDoor-1],CentLimit1|CentLimit2,DoorSwitch2,"05:30 13/03/2023","05:30 13/03/2023");
...
//    //---------------------------------Set Battery-------------------------------------------
   ...
   setBattery(BatteryHandleAndSignal[CentralBattery],CentBattery+1);
...
//----------------------------------Central GSM Signal Set----------------------------------------------

SetCentralNetwork(BatteryHandleAndSignal[CentralBattery],GSMSIG);

 //----------------------------------------Active Status SHow-----------------------------------------------
 DeviceStatusShow(RemoteDeviceStatus,1);
 DeviceStatusShow(CentralDeviceStatus,1);
 //
}
...
console.log(timecount);
    if(timecount==10)
    {
        ...
        DeviceStatusShow(CentralDeviceStatus,0);
    }
